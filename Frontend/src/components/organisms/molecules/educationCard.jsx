...
const EducationCard = ({ tenure, company, position, content }) => {
  const items = ["🔰", "🌟", "✅", "🌈", "📌"];
...
const EducationReverseCard = ({ tenure, company, position, content }) => {
  const items = ["🔰", "🌟", "✅", "🌈", "📌"];
  var item1 = items[Math.floor(Math.random()*items.length)];
  return (
    <motion.div 
...
export { EducationCard, EducationReverseCard };
